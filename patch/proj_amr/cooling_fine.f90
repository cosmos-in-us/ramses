...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  if(jeans_ncells>0)then
     !polytropic_constant = factG_in_cgs * (boxlen * jeans_ncells * 0.5d0**dble(nlevelmax) * scale_l)**2d0 / pi / gamma !GILEE
     polytropic_constant = factG_in_cgs * (jeans_ncells * dr_refine_pkpc*kpc2cm)**2d0 / pi / gamma !GILEE
  endif
  ...
  do ind=1,twotondim
     ...
     if(barotropic_eos.and.(barotropic_eos_form.ne.'legacy'))then
        ...
        if(jeans_ncells>0)then
           do i=1,nleaf
              T2min(i) = nH(i)*polytropic_constant*mH**2d0/X/kB !GILEE 
           end do
        else
           do i=1,nleaf
